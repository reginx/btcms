...
class table extends rgx {
     ...
     */
    public function get_sql ($act = 'select', $data = []) {
        if (!empty($data) && is_array($data)) {
            foreach ((array)$data as $k => $v) {
                if (isset($this->_fields[$k])) {
                    $this->data[$k] = is_callable($v) ? $v : addslashes(stripslashes($v));
                }
            }
        }
        if ($this->_error_msg) {
     ...
     * @return string
     */
    public function parse_sql ($act = false) {
        $sep = RUN_MODE == 'debug' ? " \n" : ' ';
        // action
     ...
     * @return \re\rgx\table
     */
    public function order ($str, $field_vals = []) {
        $str = trim($str);
        if ($str != '') {
            // (id, [1,3,5])
            if (!empty($field_vals)) {
                $field_vals = is_array($field_vals) ? ("'" . join("','", $field_vals) . "'") : $field_vals;
                $this->_sql['order'][] = "field({$str}, {$field_vals})";
            }
            else {
                $match = array();
