...
class table_build extends build {
    ...
    public function run () {
        ...
        if (empty($app_config['prefix'])) {
            ...
            if (empty($app_config['prefix'])) {
                $this->halt('no application directory is specified');
            }
        }

        if (!preg_match('/^\/.+/', $app_config['prefix'])) {
            $app_config['prefix'] = $this->dist_dir . "/" . $app_config['prefix'];
        }
        ...
        $db_type = strtolower($config['db']['type']);
        ...
        if (!$app_config['exec'] && 
                strtoupper($this->get_input("confirm the above information? [Y/N]", 'Y')) != 'Y') {
            $this->halt("operation terminated, Bye !");
        }
        ...
        $this->msg("complete all {$match_count} tasks" . PHP_EOL . PHP_EOL);
        exit(0);
    }
