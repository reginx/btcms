...
class imagick_image extends image {
     ...
     */
    public function captcha ($str, $config = [], $output = true) {
        ...
        $ImagickDraw->setFontSize($config['fontsize'] ?: 20);
        $config['width'] = $config['width'] ?: 85;
        
        /* Create new empty image */
        $this->_im->newImage($config['width'], $config['height'] ?: 30, $bg);

        /* Write the text on the image */
        $strlen = mb_strlen($str);
        foreach (preg_split('/\B/u', $str) as $key => $char) {
            $this->_im->annotateImage($ImagickDraw, 4 + intval($config['width'] / $strlen * $key), mt_rand(20, 26), 
                mt_rand(-30, 30), $char);
        }
